...
from common.comput_utils import check_border, IndataProcessor, midedges, segment_passes, \
    ...
    PassShuffler, dist_overlap, find_pair_times, calc_kld, append_extrinsicity, normsig, comput_overlap
...
def single_field_preprocess_emankin(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=None):
...
def single_field_preprocess_emankin_new(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=None):
    ...
    for nsess in range(num_sess):
        ...
        for ca in ['CA1', 'CA2', 'CA3']:
            ...
            for nfield in range(num_fields):
                ...
                # Construct passes (segment & chunk)
                tok, idin = tunner.get_idin(mask.T, x_ax, y_ax)
                passdf = tunner.construct_singlefield_passdf_all(tok, tsp, interpolater_x, interpolater_y,
                                                             interpolater_angle)
                direct_mask = (passdf['num_spikes']>0) & (passdf['straightrank']>sthresh)
                allchunk_df = passdf[direct_mask].reset_index(drop=True)
                ...
                neuro_keys_dict = dict(tsp='tsp', spikev='spikev', spikex='spikex', spikey='spikey',
                                       spikeangle='spikeangle')
                accept_mask = (passdf['chunked'] < 2) & (passdf['duration']>tunner.minpasstime) & (passdf['straightrank']>sthresh) & (passdf['num_spikes']>1)
                passdf['excluded_for_precess'] = ~accept_mask
                precessdf, precess_angle, precess_R, _ = get_single_precessdf(passdf, precesser, precess_filter,
                                                                              neuro_keys_dict,
                                                                              field_d=field_d, kappa=kappa_precess,
...
def pair_field_preprocess_emankin(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=None):
    ...
    for nsess in range(num_sess):
        ...
        for ca in ['CA1', 'CA2', 'CA3']:
            ...
            for npair in range(num_pairs):
                ...
                pf1, tsp1 = cell_df.loc[cellid1, ['pf', 'tsp']]
                pf2, tsp2 = cell_df.loc[cellid2, ['pf', 'tsp']]
                ...
                if (paired_tsp.shape[0] < 1) :
                    continue
                if (paired_tsp.max() - paired_tsp.min()) < 0.01:
                    continue
...
def pair_field_preprocess_emankin_new(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=None):
    pairdata_dict = dict(pair_id=[], ca=[], overlap=[],
                         border1=[], border2=[], area1=[], area2=[], xyval1=[], xyval2=[],
                         aver_rate1=[], aver_rate2=[], aver_rate_pair=[],
                         peak_rate1=[], peak_rate2=[],
    ...
    for nsess in range(num_sess):
        ...
        for ca in ['CA1', 'CA2', 'CA3']:
            ...
            for npair in range(num_pairs):
                ...
                tok1, idin1 = tunner.get_idin(mask1.T, x_ax, y_ax)
                tok2, idin2 = tunner.get_idin(mask2.T, x_ax, y_ax)
                passdf1 = tunner.construct_singlefield_passdf_all(tok1, tsp1, interpolater_x, interpolater_y, interpolater_angle)
                passdf2 = tunner.construct_singlefield_passdf_all(tok2, tsp2, interpolater_x, interpolater_y, interpolater_angle)

                direct_mask1 = (passdf1['num_spikes']>0) & (passdf1['straightrank']>sthresh)
                direct_mask2 = (passdf2['num_spikes']>0) & (passdf2['straightrank']>sthresh)

                allchunk_df1 = passdf1[direct_mask1].reset_index(drop=True)
                allchunk_df2 = passdf2[direct_mask2].reset_index(drop=True)
                ...
                # Precession1 & Post-hoc exclusion for 1st field
                accept_mask1 = (passdf1['chunked'] < 2) & (passdf1['duration']>tunner.minpasstime) & (passdf1['straightrank']>sthresh) & (passdf1['num_spikes']>1)
                passdf1['excluded_for_precess'] = ~accept_mask1
                precessdf1, precessangle1, precessR1, _ = get_single_precessdf(passdf1, precesser, precess_filter, neuro_keys_dict,
                                                                               field_d=field_d1, kappa=kappa_precess, bins=None)
                fitted_precessdf1 = precessdf1[precessdf1['fitted']].reset_index(drop=True)
                ...
                # Precession2 & Post-hoc exclusion for 2nd field
                accept_mask2 = (passdf2['chunked'] < 2) & (passdf2['duration']>tunner.minpasstime) & (passdf2['straightrank']>sthresh) & (passdf2['num_spikes']>1)
                passdf2['excluded_for_precess'] = ~accept_mask2
                precessdf2, precessangle2, precessR2, _ = get_single_precessdf(passdf2, precesser, precess_filter, neuro_keys_dict,
                                                                               field_d=field_d2, kappa=kappa_precess, bins=None)
                fitted_precessdf2 = precessdf2[precessdf2['fitted']].reset_index(drop=True)
                ...
                field_d_union = np.sqrt(mask_union.sum()/np.pi)*2
                tok_pair, _ = tunner.get_idin(mask_union.T, x_ax, y_ax)
                pairedpasses = tunner.construct_pairfield_passdf_all(tok_pair, tok1, tok2, tsp1, tsp2, interpolater_x,
                                                                 interpolater_y, interpolater_angle)

                # Phase lags
                phase_finder = ThetaEstimator(0.005, 0.3, [5, 12])
                accepted_mask = (pairedpasses['straightrank'] > sthresh) & (pairedpasses['duration'] > tunner.minpasstime)
                AB_mask = accepted_mask & (pairedpasses['direction']== 'A->B')
                BA_mask = accepted_mask & (pairedpasses['direction']== 'B->A')
                AB_tsp1_list = pairedpasses[AB_mask]['tsp1'].to_list()
                AB_tsp2_list = pairedpasses[AB_mask]['tsp2'].to_list()
                BA_tsp1_list = pairedpasses[BA_mask]['tsp1'].to_list()
                BA_tsp2_list = pairedpasses[BA_mask]['tsp2'].to_list()
                thetaT_AB, phaselag_AB, corr_info_AB = phase_finder.find_theta_isi_hilbert(AB_tsp1_list, AB_tsp2_list)
                thetaT_BA, phaselag_BA, corr_info_BA = phase_finder.find_theta_isi_hilbert(BA_tsp1_list, BA_tsp2_list)
                ...
                precess_mask = accepted_mask & (pairedpasses['chunked']<2) & \
                               ((pairedpasses['direction']=='A->B')| (pairedpasses['direction']=='B->A')) & \
                               (pairedpasses['num_spikes1']>1) & (pairedpasses['num_spikes2']>1)

                pairedpasses['excluded_for_precess'] = ~precess_mask
                precess_dfp = precesser.get_single_precession(pairedpasses, neuro_keys_dict1, field_d_union, tag='1')
                precess_dfp = precesser.get_single_precession(precess_dfp, neuro_keys_dict2, field_d_union, tag='2')
                precess_dfp = precess_filter.filter_pair(precess_dfp)
                fitted_precess_dfp = precess_dfp[precess_dfp['fitted1'] & precess_dfp['fitted2']].reset_index(drop=True)
                ...
                # Paired spikes
                direct_mask = accepted_mask & (pairedpasses['num_spikes_pair']>0)
                pairdirect_df = pairedpasses[direct_mask].reset_index(drop=True)
                paired_tsp_list = pairdirect_df['tsp_pair'].to_list()
                passangles_all = pairdirect_df['angle'].to_list()
                x_all, y_all, t_all = pairdirect_df['x'].to_list(), pairdirect_df['y'].to_list(), pairdirect_df['t'].to_list()
                ...
                if (paired_tsp.shape[0] < 1) :
                    continue
                if (paired_tsp.max() - paired_tsp.min()) < 0.0001:
                    continue
    ...
    try:
        ...
        print('Error encountered in computing extrinsicity. Save the results first.')
        pairdata.to_pickle(save_pth)
    pairdata.to_pickle(save_pth)
    return pairdata
...
def main():
    # # -------------------------- Single field ---------------------
    save_pth = 'results/emankin_python/single_field_newAllRight_previousWave.pickle'
    df = pd.read_pickle('data/emankin_pythonProcessed.pickle')
    dfprevious = pd.read_pickle('data/emankindata_processed_withwave.pickle')
    df['wave'] = dfprevious['wave']

    single_field_preprocess_emankin_new(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=save_pth)

    # # -------------------------- Pair field ------------------------
    # save_pth = 'results/emankin_python/pair_field_newAllRight.pickle'
    # df = pd.read_pickle('data/emankin_pythonProcessed.pickle')
    # # dfprevious = pd.read_pickle('data/emankindata_processed_withwave.pickle')
    # # df['wave'] = dfprevious['wave']
    # pair_field_preprocess_emankin_new(df, vthresh=5, sthresh=3, NShuffles=200, save_pth=save_pth)
